    ...
    return (
        <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="p-4 bg-white rounded-lg border text-center"><p className="text-gray-600">Positive Sentiment</p><p className="text-3xl font-bold text-green-600">{((stats.sentiments.Positive / stats.total) * 100 || 0).toFixed(0)}%</p></div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-1 p-4 bg-white rounded-lg border">
                    ...
                    <div className="w-48 h-48 mx-auto my-4 rounded-full flex items-center justify-center" style={{background: `conic-gradient(rgb(34,197,94) 0% ${((stats.sentiments.Positive/stats.total)*100).toFixed(1)}%, rgb(234,179,8) ${((stats.sentiments.Positive/stats.total)*100).toFixed(1)}% ${(((stats.sentiments.Positive+stats.sentiments.Neutral)/stats.total)*100).toFixed(1)}%, rgb(239,68,68) ${(((stats.sentiments.Positive+stats.sentiments.Neutral)/stats.total)*100).toFixed(1)}% 100%)`}}>
                ...
                <div className="lg:col-span-2 p-4 bg-white rounded-lg border">
                    ...
                    <div className="space-y-2">{stats.topTags.map(([tag, count]) => <div key={tag}><p className="text-sm font-medium">{tag}</p><div className="w-full bg-gray-200 rounded-full h-2.5"><div className="bg-indigo-600 h-2.5 rounded-full" style={{width: `${(count / stats.topTags[0][1] * 100)}%`}}></div></div></div>)}</div>
                </div>
            </div>
        </div>
    );
