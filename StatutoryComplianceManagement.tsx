    ...
    return (
        <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="p-4 bg-white rounded-lg border"><p className="text-gray-600">Compliance Status</p><div className="flex w-full h-4 rounded-md overflow-hidden mt-2"><div className="bg-green-500" style={{width: `${(stats.compliance.Compliant / stats.total) * 100}%`}}></div><div className="bg-yellow-500" style={{width: `${(stats.compliance['At Risk'] / stats.total) * 100}%`}}></div><div className="bg-red-500" style={{width: `${(stats.compliance['Non-Compliant'] / stats.total) * 100}%`}}></div></div></div>
            </div>
...
const AuditTrailView: React.FC<{ logs: ComplianceAuditLog[] }> = ({ logs }) => (
    <div className="bg-white p-4 rounded-lg border">
        ...
        <div className="space-y-2 max-h-[calc(100vh-18rem)] overflow-y-auto">
            {logs.map(log => (
                <div key={log.id} className="p-2 bg-gray-50 border-l-4">
