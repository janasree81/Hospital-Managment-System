...
import { User, Patient, LabTest, ICUNote, SOFAScore, Vital } from '../types';
...
const ICUDigitalMonitoring: React.FC<ICUDigitalMonitoringProps> = ({ user }) => {
    ...
    const VitalsCard: React.FC<{patient: Patient}> = ({patient}) => {
        ...
        if (!latestVitals) return <div className="bg-white p-4 rounded-lg shadow"><h4 className="font-bold text-gray-800 mb-2">Vitals</h4><p className="text-sm text-gray-500">No vitals recorded.</p></div>;
    
        return (
            <div className="bg-white p-4 rounded-lg shadow">
                <div className="flex justify-between items-center mb-2">
                    <h4 className="font-bold text-gray-800">Vitals</h4>
                    <p className="text-xs text-gray-500">Logged by {latestVitals.loggedBy} at {new Date(latestVitals.timestamp).toLocaleTimeString()}</p>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
                    {/* Manually list vitals for better control and formatting */}
                    <div><p className="text-gray-500">BP</p><p className={`text-lg font-semibold ${getVitalColor('bloodPressure', 0)}`}>{latestVitals.bloodPressure}</p></div>
                    <div><p className="text-gray-500">HR</p><p className={`text-lg font-semibold ${getVitalColor('heartRate', latestVitals.heartRate)}`}>{latestVitals.heartRate}</p></div>
                    <div><p className="text-gray-500">TEMP</p><p className={`text-lg font-semibold ${getVitalColor('temperature', latestVitals.temperature)}`}>{latestVitals.temperature}°F</p></div>
                    <div><p className="text-gray-500">RR</p><p className={`text-lg font-semibold ${getVitalColor('respiratoryRate', latestVitals.respiratoryRate)}`}>{latestVitals.respiratoryRate}</p></div>
                    <div><p className="text-gray-500">SPO2</p><p className={`text-lg font-semibold ${getVitalColor('oxygenSaturation', latestVitals.oxygenSaturation)}`}>{latestVitals.oxygenSaturation}%</p></div>
                    {latestVitals.map != null && <div><p className="text-gray-500">MAP</p><p className={`text-lg font-semibold ${getVitalColor('map', latestVitals.map)}`}>{latestVitals.map}</p></div>}
                    {latestVitals.cvp != null && <div><p className="text-gray-500">CVP</p><p className={`text-lg font-semibold ${getVitalColor('cvp', latestVitals.cvp)}`}>{latestVitals.cvp}</p></div>}
                </div>
            </div>
        );
    };
