...
const LabManagement: React.FC<LabManagementProps> = ({ user }) => {
    ...
    const handleSendToBilling = (testId: string) => {
        setLabTests(prev => prev.map(test =>
            test.id === testId ? { ...test, billingStatus: 'Billed' } : test
        ));
        // Also update the expanded view if it's open
         if (expandedRowId === testId) {
            setExpandedRowId(null); // Close and reopen to force re-render, simple state management
            setTimeout(() => setExpandedRowId(testId), 50);
        }
    }
    ...
    const getStatusColor = (status: LabTest['status']) => {
     ...
     const getBillingStatusColor = (status: LabTest['billingStatus']) => {
        switch (status) {
    ...
    const ReportView: React.FC<{ test: LabTest, onSendToBilling: (testId: string) => void }> = ({ test, onSendToBilling }) => (
        <div className="p-6 bg-gray-100">
            <div className="bg-white p-8 rounded-lg shadow-lg border">
                <div className="flex justify-between items-start pb-4 border-b">
                    ...
                    {test.isCritical && <div className="px-3 py-1 bg-red-100 text-red-700 font-bold rounded-full text-sm flex items-center gap-2"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.21 3.03-1.742 3.03H4.42c-1.532 0-2.492-1.696-1.742-3.03l5.58-9.92zM10 13a1 1 0 110-2 1 1 0 010 2zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" /></svg>CLINICAL ALERT</div>}
                </div>
                
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-4 py-4 border-b">
                    ...
                    <div><strong className="text-gray-600">Age / Gender:</strong> {test.age} / {test.gender}</div>
                    ...
                    <div className="lg:col-span-2"><strong className="text-gray-600">Collection Time:</strong> {new Date(test.sampleCollectionDate).toLocaleString()}</div>
                    <div><strong className="text-gray-600">Report Time:</strong> {test.reportGeneratedDate ? new Date(test.reportGeneratedDate).toLocaleString() : 'N/A'}</div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-8 pt-4 border-t">
                     <div>
                        <h5 className="font-semibold text-gray-700">Verification & Billing</h5>
                        <p className="text-sm text-gray-600 mt-1"><strong>Verified By:</strong> {test.verifiedBy || 'Pending Verification'}</p>
                        <p className="text-sm text-gray-600"><strong>Billing Status:</strong> <span className={`font-semibold ${getBillingStatusColor(test.billingStatus)} px-2 py-0.5 rounded-full`}>{test.billingStatus}</span></p>
                    </div>
                    <div className="flex justify-end items-end space-x-2">
                        {test.status === 'Completed' && test.billingStatus === 'Pending' && (
                             <button onClick={() => onSendToBilling(test.id)} className="px-3 py-1 text-sm bg-blue-600 text-white rounded-md hover:bg-blue-700">Send to Billing</button>
                        )}
            ...
            <div className="bg-white p-6 rounded-lg shadow-md">
                <div className="flex justify-between items-center mb-6">
                    ...
                    {user.role === Role.LabTech && (
                        <button onClick={() => setShowNewTestForm(!showNewTestForm)} className="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">
                            {showNewTestForm ? 'Cancel' : '+ New Test Order'}
                        </button>
                    )}
                </div>
                ...
                {showNewTestForm && (
                    <form onSubmit={handleFormSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8 p-6 border rounded-lg bg-gray-50">
                        ...
                        <div>
                            <label htmlFor="sampleCollectionDate" className="block text-sm font-medium text-gray-700">Sample Collection Time</label>
                        ...
                        <div className="col-span-full text-right">
                            <button type="submit" className="px-6 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">Create Test Order</button>
                        </div>
                    </form>
                )}
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {labTests.map(test => {
                                ...
                                <tr className={`hover:bg-gray-50 ${test.isCritical && 'bg-red-50 hover:bg-red-100'}`}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap"><span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getBillingStatusColor(test.billingStatus)}`}>{test.billingStatus}</span></td>
                                ...
                                {expandedRowId === test.id && (<tr><td colSpan={8}><ReportView test={test} onSendToBilling={handleSendToBilling} /></td></tr>)}
                                </React.Fragment>
                            )})}
                        </tbody>
                    </table>
